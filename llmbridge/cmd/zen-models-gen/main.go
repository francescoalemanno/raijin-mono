...
func render(models []zenModel) ([]byte, error) {
	...
	out.WriteString("var zenGeneratedModels = []Model{\n")
...
func writeModelLiteral(out *bytes.Buffer, model zenModel) {
	...
	if len(model.ReasoningLevels) > 0 {
		out.WriteString("\t\tReasoningLevels: []string{")
		for i, level := range model.ReasoningLevels {
			if i > 0 {
				out.WriteString(", ")
			}
			_, _ = fmt.Fprintf(out, "%q", level)
		}
		out.WriteString("},\n")
	}
	if model.DefaultReasoningEffort != "" {
		_, _ = fmt.Fprintf(out, "\t\tDefaultReasoningEffort: %q,\n", model.DefaultReasoningEffort)
	}
	_, _ = fmt.Fprintf(out, "\t},\n")
}
